...
######## EXTRACT CUSTOMER REVIEWS OF "Samsung Galaxy" ON SNAPDEAL #####################
...
####################### SENTIMENTAL ANALYSIS_SNAPDEAL REVIEWS #####################

install.packages("syuzhet")
library("syuzhet")

sg <- readLines(file.choose()) # Read Samsung galaxy review txt file
View(sg)
str(sg) # found class as charactor
sg[1:20] # See first 20 reviews

# Emotion Mining by nrc method->

nrc_sg <- get_nrc_sentiment(sg) 
head(nrc_sg,n=10) # check the scores for 20 sentenses
barplot(colSums(nrc_sg),las=2,col = rainbow(10),ylab = 'Count',main = 'Emotion Score_nrc')

get_sent <- get_sentences(sg)
class(get_sent)
str(get_sent)
head(get_sent)
...
sum(sentiment_vector) # 224
mean(sentiment_vector) # 0.83
summary(sentiment_vector) # Min: -2, Max: 3
# plot Trajectory
...
sum(afinn_s_v) # 647
mean(afinn_s_v) # 2.40
summary(afinn_s_v) # Min: -3, Max: 12
# plot Trajectory
...
head(syuzhet_vector)
sum(syuzhet_vector) # 172.85
mean(syuzhet_vector) # 0.64
summary(syuzhet_vector) # Min: -1.75, Max: 3.35
...
################ SEMENTIC ANALYSIS ON SNAPDEAL CUSTOMER REVIEW ######################
...
x = readLines(file.choose()) 	# samsung galaxy.txt
x <- stemDocument(x)
