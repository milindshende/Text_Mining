...
############# IMDB reviews Extraction "Hary Potter" ################
a<-10
Harry_Potter<-NULL
url1<-"https://www.imdb.com/title/tt0241527/reviews?start="
for(i in 1:1){
  url<-read_html(as.character(paste(url1,i*a,sep="")))
  reviews<-url %>%
    html_nodes(".show-more__control") %>%
    html_text() 
  Harry_Potter<-c(Harry_Potter,reviews)
}
##### Alternate method #####
#webpage<-read_html(url1)
#rev<-html_nodes(webpage,'.show-more__control')
#rev_text<-html_text(rev)
#head(rev_text)
###########################

write.table(Harry_Potter,file="Harry_Potter.txt")
getwd()

############# SENTIMENTAL ANALYSIS_HARRY POTTER REVIEWS oN IMDB #################

install.packages("syuzhet")
library("syuzhet")

hp <- readLines(file.choose()) # Read harry potter review txt file(as extracted above)
View(hp)
str(hp) # found class as charactor
hp[1:20] # See first 20 reviews

# Emotion Mining by nrc method->

nrc_hp <- get_nrc_sentiment(hp) 
head(nrc_hp,n=10) # check the scores for 10 sentenses
barplot(colSums(nrc_hp),las=2,col = rainbow(10),ylab = 'Count',main = 'Emotion Score_nrc')

get_sent <- get_sentences(hp)
class(get_sent)
str(get_sent)
head(get_sent)
...
sum(sentiment_vector) # 304
mean(sentiment_vector) # 0.53
...
sum(afinn_s_v) # 714
mean(afinn_s_v) # 1.25
summary(afinn_s_v) # Min: -6, Max: 13
# plot Trajectory
...
head(syuzhet_vector)
sum(syuzhet_vector) # 348.85
mean(syuzhet_vector) # 0.61
summary(syuzhet_vector) # Min: -2.35, Max: 4.45
...
############# SEMENTIC ANALYSIS ON HARRY POTTER REVIEW ON IMDB ####################
...
x = readLines(file.choose()) 	# harry potter.txt
x <- stemDocument(x)
